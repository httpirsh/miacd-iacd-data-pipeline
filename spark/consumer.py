...
def perform_clustering(df, k=3):
    """HYBRID: K-means + relabel by CO2 (guarantees Low/Mid/High)"""
    ...
    # calculate Silhouette Score (before relabeling)
    evaluator = ClusteringEvaluator(featuresCol="scaled_features", 
                                     predictionCol="cluster_original",
                                     metricName="silhouette")
    silhouette_score = evaluator.evaluate(results)

    # Relabel clusters by average CO2 (Low=0, Mid=1, High=2)
    cluster_avg_co2 = results.groupBy("cluster_original").agg(
        avg("avg_co2").alias("avg_co2_in_cluster")
    ).collect()

    # Sort by CO2: lowest→0, middle→1, highest→2
    ...
    # Apply relabeling
    mapping_expr = when(col("cluster_original") == cluster_mapping[0][0], relabel_map[cluster_mapping[0][0]])
    for i in range(1, len(cluster_mapping)):
        mapping_expr = mapping_expr.when(col("cluster_original") == cluster_mapping[i][0], relabel_map[cluster_mapping[i][0]])

    results = results.withColumn("cluster", mapping_expr)

    # Print cluster info
    labels = {0: "Low", 1: "Mid", 2: "High"}
    print(f"\n{k} clusters for {cleaned_data.count()} countries (Silhouette: {silhouette_score:.4f})")
    print("Cluster labels (by avg CO2):")
    for orig_id, new_id in sorted(relabel_map.items(), key=lambda x: x[1]):
        avg_co2_val = next(x[1] for x in cluster_mapping if x[0] == orig_id)
        print(f"  {new_id} ({labels[new_id]} emitters): avg CO2 = {avg_co2_val:.2f} MT")
...
def show_cluster_results(results, k):
    ...
    for cluster_id in range(k):
        ...
        print(f"cluster {cluster_id} ({labels[cluster_id]} emitters, top 5 by CO2):")
...
def process_clustering(batch_df, batch_id):
    ...
    try:
        ...
        cluster_stats_db = cluster_stats.withColumn("batch_id", lit(batch_id)) \
                                         ...
                                         .select(
                                             "batch_id", "cluster", "num_countries",
